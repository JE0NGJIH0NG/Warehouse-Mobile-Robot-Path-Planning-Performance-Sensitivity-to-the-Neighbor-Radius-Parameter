...
NAV_CONFIG_PATH=/home/jihong/frrt/nav2.yaml

# === Nav2 노드 실행 ===
gnome-terminal -- bash -c "
  echo '$IDENTIFIER_MAP_SERVER';
  ros2 run nav2_map_server map_server --ros-args -p yaml_filename:=/home/jihong/frrt/my_map.yaml -p use_sim_time:=true;
  exec bash
" &
...
gnome-terminal -- bash -c "
  ...
  ros2 launch nav2_bringup rviz_launch.py use_sim_time:=true default_nav_to_pose_bt_xml:=/home/jihong/frrt/navigate_through_poses_custom.xml;
  exec bash
" &
...
# ros2 launch turtlebot4_ignition_bringup turtlebot4_ignition.launch.py slam:=false nav2:=false rviz:=false use_sim_time:=true x:=-13.00 y:=23.00 yaw:=2.50 &
